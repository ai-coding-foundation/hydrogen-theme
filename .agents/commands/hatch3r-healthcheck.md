...
1. Add to Projects v2 board via `projects_write` with `method: add_project_item`
2. Update status via `projects_write` with `method: update_project_item` (set to Ready using project's status field option ID)
...
1. **Projects v2 Sync:** Follow the **Projects v2 Sync Procedure** from the shared context for the healthcheck epic and ALL sub-issues. Set status to Ready using the project's status field option ID.
...
- `projects_write` failure: warn and continue. Board sync can be fixed later via board-refresh.

## Guardrails

- **Never skip ASK checkpoints.**
- **Use GitHub MCP tools exclusively** (no `gh` CLI for issue operations).
